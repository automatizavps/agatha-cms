...
const OrderForm: React.FC<OrderFormProps> = ({ onSubmit, isSubmitting, defaultValues, isEditing = false }) => {
  ...
  const handleAddItem = () => {
    ...
    if (firstItem) {
      ...
      append({ 
        produto_id: firstItem.id, 
        quantidade: initialQuantity, 
        preco_unitario: firstItem.preco 
      });
      
      // 4. Força a revalidação do novo item para garantir que o botão seja habilitado
      // O índice do novo item é fields.length (antes do append)
      const newIndex = fields.length;
      form.trigger(`items.${newIndex}.produto_id`);
      form.trigger(`items.${newIndex}.quantidade`);
      
    } else {
      // Se não houver itens, adiciona um item vazio (o formulário exibirá o erro de validação)
  ...
  const handleProductChange = (index: number, productId: string) => {
    ...
    if (selectedItem) {
      ...
      form.trigger(`items.${index}.produto_id`); 
    }
  };
