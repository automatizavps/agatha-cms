...
import { updatePromotion, Promotion, usePromotionRules, PromotionRule } from "@/integrations/supabase/promotions";
...
const EditPromotionSheet: React.FC<EditPromotionSheetProps> = ({ promotion, isOpen, onOpenChange }) => {
  ...
  const handleSubmit = (values: { 
    ...
    mutation.mutate({ id: promotion.id, ...values });
  };
    ...
    return (
      <Sheet open={isOpen} onOpenChange={onOpenChange}>
        <SheetContent className="sm:max-w-xl flex flex-col">
          <SheetHeader>
            <SheetTitle>{t('loading_promotion_data', { defaultValue: 'Carregando Promoção...' })}</SheetTitle>
